...
def send_daily_mission_notifications():
    ...
    for user in users:
        try:
            daily_info = DailyInfo.objects.get(user=user, date=datetime.date.today)
            ...
            # If there is no DailyInfo record for today, we assume the missions are not completed.
            message = "오늘의 미션을 완료했는지 확인해 보세요!"
            url_text = "미션 확인하러 가기"
            Notification.objects.create(user=user, message=message, url_text=url_text)
...
scheduler.add_job(send_acupressure_point_notifications, 'interval', minutes=1, start_date='2024-08-04 00:08:00', id='acupressure_point')
